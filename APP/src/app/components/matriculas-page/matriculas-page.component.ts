...
export class MatriculasPageComponent {
    ...
    async excluir(matriculaId: number): Promise<void> {
      ...
      try {
        const response = await this.authService.authenticatedFetch(`/matriculas/${matriculaId}`, {
          method: 'DELETE'
        });
    ...
    private async atualizar(): Promise<void> {
      const response = await this.authService.authenticatedFetch(`/matriculas/${this.formData.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    private validarFormulario(): boolean {
      ...
      if (this.formData.idAluno == 0) {
        this.errorMessage = 'Id do Aluno é obrigatório';
        return false;
      }
      return true;
    }
